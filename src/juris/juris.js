        ...
        cleanup(element) {
            ...
            if (data) {
                data.subscriptions?.forEach(unsubscribe => {
                    try { unsubscribe(); } catch (error) { console.warn(log.w('Error during subscription cleanup:', error), 'application'); }
                });
                data.eventListeners?.forEach(({ eventName, handler }) => {
                    try { element.removeEventListener(eventName, handler); } catch (error) { console.warn(log.w('Error during event listener cleanup:', error), 'application'); }
                });
            ...
            try {
                Array.from(element.children || []).forEach(child => {
                    try { this.cleanup(child); } catch (error) { console.warn(log.w('Error cleaning up child element:', error), 'application'); }
                });
            } catch (error) {
                console.warn(log.w('Error during children cleanup:', error), 'application');
            }
        }
